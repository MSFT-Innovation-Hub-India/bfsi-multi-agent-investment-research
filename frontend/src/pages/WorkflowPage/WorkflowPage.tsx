...
function StatusBadge({ status }: { status: 'pending' | 'processing' | 'completed' }) {
  const statusConfig = {
    completed: { bg: '#126125', text: 'white', label: '✓ Analysis Complete', border: '#126125' },
    processing: { bg: '#00246B', text: '#CADCFC', label: '⟳ Processing...', border: '#00246B' },
    pending: { bg: '#9ca3af', text: 'white', label: '⏸ Pending', border: '#9ca3af' }
  }
...
function AgentCard({ name, icon, status, description, output, metrics, tasks }: AgentCardProps) {
  ...
  const statusStyles = {
    completed: { border: '#126125', shadow: '0 0 30px rgba(18, 97, 37, 0.4)', opacity: 1, scale: 1 },
    processing: { border: '#00246B', shadow: '0 0 30px rgba(0, 36, 107, 0.4)', opacity: 1, scale: 1.05 },
    pending: { border: '#CADCFC', shadow: 'none', opacity: 0.6, scale: 1 }
  }
  ...
  return (
    <div 
      className="relative rounded-2xl border-2 transition-all duration-500 transform bg-white"
      style={{
      ...
      <div className="relative p-6 z-10">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            <h3 className="font-bold text-xl mb-1 text-[#00246B]">{name}</h3>
            <p className="text-sm mb-3 text-[#00246B]/80">{description}</p>
        ...
        {output && status === 'completed' && (
          <div className="mt-4">
            <button
              ...
              className="flex items-center gap-2 text-sm font-medium transition-colors text-[#00246B]"
            >
              <Eye className="h-4 w-4" />
              {expanded ? 'Hide Details' : 'Show Details'}
            </button>
            {expanded && (
              <div className="mt-3 p-5 rounded-xl border-2 text-sm leading-relaxed shadow-lg bg-[#00246B] text-[#CADCFC] border-[#CADCFC]">
                <div className="mb-4 pb-2 border-b border-[#CADCFC] opacity-50">
                ...
                {metrics && (
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    {metrics.map((metric, idx) => (
                      <div key={idx} className="rounded-xl p-4 border-2 shadow-md bg-[#CADCFC] border-[#CADCFC]">
                        <div className="text-xs mb-1 font-medium uppercase tracking-wide text-[#00246B] opacity-60">
                          {metric.label}
                        </div>
                        <div className="text-xl font-bold text-[#00246B]">{metric.value}</div>
                      </div>
                    ))}
                  </div>
                )}
...
function WorkflowPage() {
  ...
  useEffect(() => {
    // Load extracted workflow metrics
    fetch('/data/workflow_metrics.json')
      .then(r => r.json())
      .then(data => {
  ...
  const startWorkflow = async () => {
    ...
    try {
      ...
      const es = new EventSource(`${apiUrl}${data.stream_url}`)
      setEventSource(es)
  ...
  return (
    <div className="min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-8">
        <div className="rounded-2xl p-6 shadow-lg mb-8 bg-white border-2 border-[#00246B]">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-[#00246B]">GMR Airports Investment Analysis</h2>
          ...
          <div className="flex items-center gap-4 mb-4">
              <div className="p-3 rounded-lg bg-[#00246B]">
              <Zap className="h-6 w-6 text-[#CADCFC]" />
            </div>
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <span className="text-lg font-semibold text-[#00246B]">GMR Airports - Analysis Progress</span>
                <span className="text-2xl font-bold text-[#00246B]">{Math.round(progress)}%</span>
              </div>
              <div className="w-full rounded-full h-4 overflow-hidden bg-[#CADCFC]">
                <div 
                  className="h-4 rounded-full transition-all duration-500 relative overflow-hidden bg-[#00246B]"
          ...
          <div className="flex items-center gap-6 text-sm text-[#00246B]">
        ...
        {progressLogs.length > 0 && (
          <div className="mt-8 rounded-2xl p-6 shadow-lg border bg-white border-[#00246B]">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2 text-[#00246B]">
